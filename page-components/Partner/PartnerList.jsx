...
import { usePartners, usePartnerPages } from "../../lib/partner";
...
const classes = {
  ...
  modalContainer:
    "flex items-center justify-end p-6 border-t border-solid border-pinkGray-200 rounded-b",
  modalClose:
    "text-red-500 background-transparent font-bold uppercase px-6 py-2 text-sm outline-none focus:outline-none mr-1 mb-1",
  modalSave:
    "text-white bg-pink-500 active:bg-pink-700 font-bold uppercase text-sm px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1",
};
...
const PartnerList = () => {
  ...
  const partners =
    data?.[0]?.partners.length === 0 || data?.[0]?.length === 0
      ? []
      : data.reduce((acc, val) => [...acc, ...val.partners], []);
  return (
    <div>
          ...
          <div className="overflow-x-auto sm:-mx-6 lg:-mx-8">
            <div className="container container m-auto flex flex-wrap flex-col md:flex-row items-center justify-end">
              {partners.map((partner, id) => (
                <div
                  key={id}
                  className="shadow-lg flex flex-wrap w-full lg:w-4/5 mx-auto border-b-4"
                >
                  <div className="flex flex-col lg:flex-row rounded overflow-hidden h-auto lg:h-45">
                    ...
                    <div className="bg-white rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
                      ...
                      <div className="text-gray-400 font-normal text-md mb-1 leading-tight">
                        {partner.content}
                      </div>
