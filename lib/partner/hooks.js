...
export function usePartners(limit) {
  const { data, error } = useSWR(
    `/api/partners?limit=${limit ?? Number.MAX_SAFE_INTEGER}`,
    fetcher
  );
  return {
    data,
...
export function usePartnerPages({ partnerId, limit = 5 } = {}) {
  const { data, error, size, ...props } = useSWRInfinite(
    (index, previousPageData) => {
      // reached the end
      if (previousPageData && previousPageData.partners.length === 0)
        return null;
      ...
      if (index !== 0) {
        ...
        const before = new Date(
          new Date(
            previousPageData.partners[
              previousPageData.partners.length - 1
            ].createdAt
          ).getTime()
        );
