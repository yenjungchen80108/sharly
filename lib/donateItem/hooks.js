...
export function useDonateItems() {
  const { data, error } = useSWR('/api/donateItems', fetcher);
  return {
    donateItemData: data,
...
export function useDonateItem({ donateItemId }) {
  const { donateItemData, error } = useSWR(`/api/donateItems/${partnerId}`, fetcher);

  return {
    donateItemData,
    isLoading: !error && !donateItemData,
    isError: error
  };
}

export function useDonateItemsPages(index) {
  const { data, size, setSize, error } = useSWR(`/api/donateItems?page=${index}`, fetcher,
    {
      initialData: data,
